 ...
 * Substituir 'corte' pelo nome do setor (ex: corte, dobra, solda)
 */
...
function getMetaPrazoSetor(?string $dataTermino, string $etapaAtual): array
{
    if (empty($dataTermino) || !in_array($etapaAtual, ['corte', 'dobra'], true)) {
        return ['classe' => '', 'rotulo' => '', 'mensagem' => ''];
    }

    try {
        $hoje = new DateTimeImmutable(date('Y-m-d'));
        $prazo = new DateTimeImmutable((new DateTimeImmutable($dataTermino))->format('Y-m-d'));
        $diasRestantes = (int) $hoje->diff($prazo)->format('%r%a');
        ...
        if ($diasRestantes < 0) {
            ...
            return [
                'classe' => 'danger',
                'rotulo' => 'Atrasada',
                'mensagem' => "Atenção: esta O.S. está atrasada em {$diasAtraso} dia(s) e ainda não saiu do {$etapaAtual}.",
            ];
        }
        ...
        if ($diasRestantes <= 5) {
            return [
                'classe' => 'warning',
                'rotulo' => 'Prazo crítico',
                'mensagem' => "Aviso de prazo: faltam {$diasRestantes} dia(s) para a entrega e esta O.S. ainda não avançou do {$etapaAtual}.",
            ];
        }
    } catch (Throwable $e) {
        return ['classe' => '', 'rotulo' => '', 'mensagem' => ''];
...
foreach ($ordens as &$ordemSetor) {
    $metaPrazo = getMetaPrazoSetor($ordemSetor['data_termino'] ?? null, (string) ($ordemSetor['etapa_atual'] ?? ''));
    $ordemSetor['prazo_alerta_classe'] = $metaPrazo['classe'];
    $ordemSetor['prazo_alerta_rotulo'] = $metaPrazo['rotulo'];
    $ordemSetor['prazo_alerta_mensagem'] = $metaPrazo['mensagem'];
}
unset($ordemSetor);
...
<div class="vend-layout">
    ...
    <div class="vend-main"><div class="vend-page-head"><h1 class="vend-page-title">Corte</h1></div><div class="vend-content">
<style>
.modal {
    display: none;
    position: fixed;
                        ...
                        <?php foreach ($ordens as $os): ?>
                            <tr>
                                <td>
                                    ...
                                    <?php if (!empty($os['prazo_alerta_rotulo'])): ?>
                                        <div style="margin-top:6px;">
                                            <span class="vbadge-<?php echo htmlspecialchars($os['prazo_alerta_classe']); ?>">
                                                <?php echo htmlspecialchars($os['prazo_alerta_rotulo']); ?>
                                            </span>
                                        </div>
                                    <?php endif; ?>
                                </td>
...
function getAlertaPrazoProducao(os) {
    if (os && os.prazo_alerta_mensagem) {
        const classe = os.prazo_alerta_classe === 'danger' ? 'danger' : 'warning';
        const bg = classe === 'danger' ? '#fef2f2' : '#fffbeb';
        const border = classe === 'danger' ? '#dc2626' : '#f59e0b';
        const color = classe === 'danger' ? '#991b1b' : '#92400e';
        return `
            <div style="margin-bottom:12px;padding:12px 14px;border-left:4px solid ${border};background:${bg};color:${color};border-radius:6px;">
                ${escapeHtml(os.prazo_alerta_mensagem)}
            </div>
        `;
    }

    if (!os || !os.data_termino || !['corte', 'dobra'].includes(os.etapa_atual)) {
        return '';
    }
    ...
    const dataBase = String(os.data_termino).split(' ')[0];
    const prazo = new Date(`${dataBase}T00:00:00`);
    ...
    if (diffDias < 0) {
        return `
            <div style="margin-bottom:12px;padding:12px 14px;border-left:4px solid #dc2626;background:#fef2f2;color:#991b1b;border-radius:6px;">
                Atenção: esta O.S. está atrasada em ${Math.abs(diffDias)} dia(s) e ainda não saiu do ${escapeHtml(os.etapa_atual)}.
            </div>
        `;
    }
    ...
    if (diffDias <= 5) {
        return `
            <div style="margin-bottom:12px;padding:12px 14px;border-left:4px solid #f59e0b;background:#fffbeb;color:#92400e;border-radius:6px;">
                Aviso de prazo: faltam ${diffDias} dia(s) para a entrega e esta O.S. ainda não avançou do ${escapeHtml(os.etapa_atual)}.
            </div>
        `;
    }
...
function abrirModal(os) {
    ...
    fetch('<?php echo SITE_URL; ?>/api/os.php?id=' + os.id)
        .then(response => response.json())
        .then(data => {
            const osDetalhe = data.os || os;
            const alertaPrazo = getAlertaPrazoProducao({
                ...os,
                ...osDetalhe
            });
            ...
            let html = `
                ...
                <p><strong>Data de Entrega:</strong> ${formatDateEntregaBr(osDetalhe.data_termino || os.data_termino)}</p>
                ${alertaPrazo}
                ${blocoRecall}
                <hr>
                <h5>Observações Técnicas:</h5>
                <p><strong>Corte/Dobra:</strong> ${escapeHtml(osDetalhe.observacoes_corte_dobra || 'Nenhuma')}</p>
                <p><strong>Solda:</strong> ${escapeHtml(osDetalhe.observacoes_solda || 'Nenhuma')}</p>
                <hr>
            `;
